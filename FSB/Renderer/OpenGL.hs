...
drawWorld renderer world (Sky sky stars clouds) = do
  ...
  forM_ clouds $ \(Cloud w h pos) ->
    forM_ (map (moveRelativeTo $ over both round pos) $ cloudRects w h) $ draw (255, 255, 255)
